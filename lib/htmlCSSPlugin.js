...
async function resizeImage(path, width) {
  console.log("Resizing", path, "to", width);
  try {
    return await sharp(join(".build-tmp", path))
      .resize(parseInt(width, 10))
      .toBuffer();
  } catch (_err) {
    return await sharp(join("src", path))
      .resize(parseInt(width, 10))
      .toBuffer();
  }
}
  ...
  async function processContent(rollup, filePath) {
    ...
    content = replaceAsync(
      ...
      async (_fullMatch, width, _quote, path) => {
        ...
        if (!isImage)
          throw new Error(`img-${width} imports must use valid image format.`);
        ...
        if (!imgCache.has(newPath)) {
          const source = await resizeImage(targetPath, width);
          const id = rollup.emitAsset(newPath, source);
          imgCache.set(newPath, id);
        }

        return `${importMetaStart}import.meta.ROLLUP_ASSET_URL_${imgCache.get(
          newPath
        )}${importMetaEnd}`;
      }
    );
